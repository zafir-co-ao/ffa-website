...
const event = await loadEvent(fid, $locale.value);
...
<template>
	<div class="container">
		<div v-if="event">
			<Title>{{ event.title }} - {{ strings.meta_title[$locale] }}</Title>
			...
			<app-article-breadcrumbs
				...
				:third-level-title="strings.events[$locale]"
			...
			<div v-if="event.registrationUrl" class="mt-5">
				{{ strings.online_registration[$locale] }}:
