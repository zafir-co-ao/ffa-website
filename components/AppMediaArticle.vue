...
<template>
	<div class="publication-component mt-5 cursor-pointer">
		<div v-if="article">
			<div class="body2 color-gray">{{ article.publishedOn }} | {{ article.publicationName }}</div>
			<p class="h5 text-black font-weight-500 mb-2">{{ article?.title }}</p>
			<div class="body2 azul mb-2">
				<nuxt-link class="color-blue" v-if="article.lawyerUuid" :to="`/${lang}/lawyers/${lawyerFid}`">
					{{ article.lawyerName }}
				</nuxt-link>
				<a class="color-blue" v-else-if="article.lawyerLinkedIn" :href="article.lawyerLinkedIn" target="_blank">
					{{ article.lawyerName }}
				</a>
			...
			<div class="body2">
				<nuxt-link :to="`/${lang}/media-articles/${article.fid}`"> {{ strings.know_more[lang] }}</nuxt-link>
			</div>
		</div>
	</div>
</template>
